...
const getOrderlistCount = async (req, res) => {
  try {
    const { userId } = req.user;
    const { orderlistStatus } = req.query;
    let whereClause = { userId };
    if (orderlistStatus) {
      whereClause = { orderlistStatus, userId };
    }
    const orderlistCount = await All_Models.Orderlist_Model.count({
      where: whereClause,
    });
    res.status(200).json({
      message: "Orderlist Count",
      data: orderlistCount,
    });
  } catch (error) {
    res.status(500).json({
